...
class ConfigParserTest {
	...
	@Test
	fun `parse input prefers android keyboard button key`() {
		val file = writeConfig("""
			vkbd_enabled=false
			input.default_osk=false
			amiberry.show_android_keyboard_button=true
	...
	@Test
	fun `sanitize unknown lines removes handled touch control keys`() {
		val lines = listOf(
			"onscreen_joystick=true",
			"vkbd_enabled=true",
			"input.default_osk=false",
			"custom_option=keepme"
		)

		val sanitized = ConfigParser.sanitizeUnknownLines(lines)

		assertEquals(listOf("custom_option=keepme"), sanitized)
	}

	@Test
	fun `migrate legacy managed settings disables stale onscreen joystick`() {
		val file = tempDir.newFile(".last_session.uae")
		file.writeText("""
			amiberry.onscreen_joystick=true
			amiberry.android_joyport1=onscreen_joy
		""".trimIndent())

		val migrated = ConfigParser.migrateLegacyManagedSettings(
			file,
			EmulatorSettings(onScreenJoystick = true, joyport1 = "onscreen_joy")
		)

		assertFalse(migrated.onScreenJoystick)
		assertEquals("joy1", migrated.joyport1)
	}

	@Test
	fun `migrate managed settings keeps versioned configs unchanged`() {
		val file = tempDir.newFile(".last_session.uae")
		file.writeText("${UpstreamConfig.KEY_TOUCH_SETTINGS_VERSION}=${UpstreamConfig.TOUCH_SETTINGS_VERSION}")

		val original = EmulatorSettings(onScreenJoystick = true, joyport1 = "onscreen_joy")
		val migrated = ConfigParser.migrateLegacyManagedSettings(file, original)
