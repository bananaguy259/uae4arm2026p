...
```
android/app/src/main/java/com/uae4arm2026/
├── [Activities]
│   ├── ui/MainActivity.kt          # Compose entry: asset extraction + theme
│   └── Uae4ArmEmulatorActivity.java # SDL Activity in :sdl process
│
├── [Navigation]
│   ├── ui/Uae4ArmApp.kt            # Root Compose: responsive nav (rail/bar)
...
│   ├── data/AppStorage.kt          # UAE4Arm-owned preferences with legacy migration
│   └── data/UpstreamConfig.kt      # Stable upstream Amiberry Android config keys
│
└── [Models]
...
| Add new screen | `Screen.kt` + `Uae4ArmApp.kt` | Sealed class + NavHost route |
...
- **Ownership split**: UI/storage/app prefs are UAE4Arm-owned; upstream-required config keys stay centralized in `UpstreamConfig.kt`
